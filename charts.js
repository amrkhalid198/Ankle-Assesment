...
const RadarChart = {
  ...
  draw(canvasId, data, options = {}) {
    ...
    // getBoundingClientRect returns 0×0 when the canvas hasn't been laid out
    // yet (e.g. the screen just transitioned in). Fall back to a safe fixed
    // size so the chart always renders — the CSS will scale it visually.
    const rect = canvas.getBoundingClientRect();
    const cssW = rect.width  || canvas.offsetWidth  || 300;
    const cssH = rect.height || canvas.offsetHeight || 300;
